...
def normalize_pe_rebase0(data: bytes) -> Optional[bytes]:
    ...
    Zeroes OptionalHeader.ImageBase, FileHeader.TimeDateStamp and
    OptionalHeader.CheckSum, and reverses every base relocation fixup, so the
    returned bytes depend only on the module's code -- not on where it was
    placed or when it was built. Returns None for TE / non-PE / unparsable or
    unsupported input (skip, never fake).
    """
    try:
        ...
        # CheckSum sits at OptionalHeader+64 in both PE32 and PE32+.
        if opt + 68 > len(data):
            return None

        buf = bytearray(data)
        # Zero the three header fields that vary without the code varying.
        # TimeDateStamp and CheckSum matter as much as ImageBase: leave the
        # timestamp in the preimage and two builds of identical source hash
        # differently, which defeats the layout-independent identity this
        # function exists to provide. (On images produced by edk2's GenFw these
        # two are already zero, so this is a no-op there -- but that is a
        # property of one toolchain, not of PE.)
        ...
        while pos + 8 <= end:
            ...
            if block_size < 8 or pos + block_size > end or (block_size - 8) % 2:
                # A malformed block is a failure, not the end of the walk: breaking here
                # would return a partially-normalized image that looks valid. Emit no value.
                return None
